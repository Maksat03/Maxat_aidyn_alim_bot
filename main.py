...
async def inline_echo(call: CallbackQuery):
	if call.message:
		...
		if call.data.startswith("ctgr_"):
			...
			await bot.send_message("732957488", msg_text, reply_markup=get_inline_keyboard_markup([["✅ Принят заказ", f"accept_order:{call.message.chat.id}"]]))
			...
			if "latitude" in msg_text[1] and "longitude" in msg_text[1]:
				...
				await bot.send_location("732957488", latitude, longitude)
